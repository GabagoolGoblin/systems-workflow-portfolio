...
For the public copy, exactly one locally added request-contact field was removed at JSON pointer `/user_agent`. The resulting file is not described as the exact raw capture. `data/DATA_REDACTION_RECEIPT.json` binds the original and public SHA-256 digests, the one removal operation, the canonical hash of every remaining field, and the retained-record hash. No replacement or fictitious contact was inserted.

Owner-created material is governed by the common root `LICENSE`, and its provenance is recorded in `release-decisions.json`. The third-party data paths remain under their stated ODbL/DbCL terms.
