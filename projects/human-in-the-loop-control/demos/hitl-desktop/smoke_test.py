...
def main() -> int:
    ...
    cache_resolution_ok = hits == 3 and held == 1
    staging_blocked = False
    try:
        stage_price_updates(rows)
    except ValueError:
        staging_blocked = True
    held_row = next(row for row in rows if not row.menu_item_id)
    validate_held_row(rows, cache, held_row.barcode, HELD_MANUAL_MENU_ITEM_ID)
    stage_price_updates(rows)
    ...
    held_row = next(row for row in mismatch_rows if not row.menu_item_id)
    validate_held_row(
        mismatch_rows, mismatch_cache, held_row.barcode, HELD_MANUAL_MENU_ITEM_ID
    )
    stage_price_updates(mismatch_rows)
    mismatch_blocked = not reread_staged_updates(
        mismatch_rows, mismatch_barcode=mismatch_rows[0].barcode
    )
    save_blocked = False
    try:
